...
export default function SuperPriceClient({
  ...
  // 1페이지는 8개, 2페이지는 10개 상품 출력
  const startIdx = page * 8;
  const endIdx = page === 0 ? 8 : 18;
  const currentProducts = initialProducts.slice(startIdx, endIdx);
  ...
  return (
    <section className="mx-auto flex h-auto w-[1280px] flex-col gap-[18px] py-[40px]">
      {/* 상단 텍스트 및 전체보기 */}
      ...
      {/* 중앙 그리드 */}
      <div
        className={`mt-[2px] w-full ${page === 0 ? 'grid grid-cols-[243px_1fr] gap-[16px]' : 'block'}`}
      >
        {page === 0 && (
          <div
            className="relative h-[710px] w-full cursor-pointer overflow-hidden rounded-[8px] bg-blue-600"
        ...
        {/* 우측 상품 영역 */}
        <div
          className={`grid gap-x-[16px] gap-y-[20px] ${page === 0 ? 'grid-cols-4' : 'grid-cols-5'}`}
        >
